  ...
  return (
    <div className='flex flex-col w-full bg-grey'>
        { textImageData.map((item) => {
          return (
              <div key={item.id} className='flex flex-col md:flex-row text-center items-center'>
                <div className={`flex flex-col md:w-1/2 py-8 w-full h-full items-center order-last ${item.type === "A" ? "md:order-first" : "md:order-last"}`}>
                  <h1 className='font-bold text-3xl md:text-5xl py-2'>{item.title}</h1>
                  <p className='items-center px-4 md:px-12 py-12 text-xl'>{item.desc}</p>
                  <Link to={item.link} className='py-4 px-8 rounded-md text-lg font-bold bg-darkGrey hover:bg-darkGreyHover text-white'>{item.btnText}</Link>
                </div>
